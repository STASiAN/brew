...
brew install asciinema awscli bat binwalk coreutils cowsay dep direnv dive findutils fping gawk git git-flow gnu-sed gnu-tar go hashcat helm httpie httpstat ipcalc iperf jq k9s krew kubectl kubectx kubespy lolcat lsd mas midnight-commander mtr ncdu neofetch nmap p7zip packer pidof ponysay pv pwgen qrencode radare2 shellcheck sl socat sshfs stern tektoncd-cli telnet tig tmux tree watch wget wireguard-go wireguard-tools xz yara yq zsh-completion
