...
brew cask install aerial awscli docker osxfuse slack sourcetree telegram-desktop tor-browser transmission visual-studio-code vlc wireshark
brew install asciinema bash-completion@2 bat binwalk cowsay direnv fping gawk git git-flow gnu-tar go hashcat httpie ipcalc iperf istioctl jq kubectx kubernetes-cli kubernetes-helm kubespy lolcat lsd mas midnight-commander mtr ncdu neofetch nmap p7zip pidof pv pwgen radare2 shellcheck socat sshfs stern telnet tig tmux unrar watch wget wireguard-go wireguard-tools xhyve xz yara zsh-completion
